...
var _RequireInTheMiddleSingleton = require('./RequireInTheMiddleSingleton');

var _importInTheMiddle = require('import-in-the-middle');

var _api = require('@opentelemetry/api');

var _requireInTheMiddle = require('require-in-the-middle');
 ...
 */
var InstrumentationBase = exports.InstrumentationBase = function (_InstrumentationAbstr) {
    _inherits(InstrumentationBase, _InstrumentationAbstr);
    ...
    function InstrumentationBase(instrumentationName, instrumentationVersion, config) {
        ...
        _this._hooks = [];
        _this._requireInTheMiddleSingleton = _RequireInTheMiddleSingleton.RequireInTheMiddleSingleton.getInstance();
        _this._enabled = false;
        _this._wrap = function (moduleExports, name, wrapper) {
            if ((0, _utils.isWrapped)(moduleExports[name])) {
                _this._unwrap(moduleExports, name);
            }
            if (!_util.types.isProxy(moduleExports)) {
                return (0, _shimmer.wrap)(moduleExports, name, wrapper);
            } else {
                var wrapped = (0, _shimmer.wrap)(Object.assign({}, moduleExports), name, wrapper);
        ...
        _this._unwrap = function (moduleExports, name) {
            if (!_util.types.isProxy(moduleExports)) {
                return (0, _shimmer.unwrap)(moduleExports, name);
            } else {
                return Object.defineProperty(moduleExports, name, {
        ...
        _this._massWrap = function (moduleExportsArray, names, wrapper) {
            if (!moduleExportsArray) {
                _api.diag.error('must provide one or more modules to patch');
                return;
            } else if (!Array.isArray(moduleExportsArray)) {
                moduleExportsArray = [moduleExportsArray];
            }
            if (!(names && Array.isArray(names))) {
                _api.diag.error('must provide one or more functions to wrap on modules');
                return;
            }
        ...
        if (_this._modules.length === 0) {
            _api.diag.debug('No modules instrumentation has been defined for ' + ('\'' + _this.instrumentationName + '@' + _this.instrumentationVersion + '\'') + ', nothing will be patched');
        }
    ...
    _createClass(InstrumentationBase, [{
        ...
        value: function _extractPackageVersion(baseDir) {
            try {
                var json = (0, _fs.readFileSync)(path.join(baseDir, 'package.json'), {
                    encoding: 'utf8'
                });
                var version = JSON.parse(json).version;
                return typeof version === 'string' ? version : undefined;
            } catch (error) {
                _api.diag.warn('Failed extracting version', baseDir);
            }
            return undefined;
        }
    }, {
        ...
        value: function enable() {
            ...
            var _loop = function _loop(_module) {
                ...
                var hook = path.isAbsolute(_module.name) ? new _requireInTheMiddle.Hook([_module.name], { internals: true }, onRequire) : _this4._requireInTheMiddleSingleton.register(_module.name, onRequire);
                _this4._hooks.push(hook);
                var esmHook = new _importInTheMiddle.Hook([_module.name], { internals: false }, hookFn);
...
function isSupported(supportedVersions, version, includePrerelease) {
    ...
    return supportedVersions.some(function (supportedVersion) {
        return (0, _semver.satisfies)(version, supportedVersion, { includePrerelease: includePrerelease });
    });
}
