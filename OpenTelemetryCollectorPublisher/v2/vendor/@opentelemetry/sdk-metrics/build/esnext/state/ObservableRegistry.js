 ...
 */
var ObservableRegistry = exports.ObservableRegistry = function () {
    ...
    _createClass(ObservableRegistry, [{
        ...
        value: function addBatchCallback(callback, instruments) {
            ...
            var observableInstruments = new Set(instruments.filter(_Instruments.isObservableInstrument));
            if (observableInstruments.size === 0) {
                _api.diag.error('BatchObservableCallback is not associated with valid instruments', instruments);
                return;
            }
                    ...
                    while (1) {
                        switch (_context.prev = _context.next) {
                            case 0:
                                ...
                                _context.next = 4;
                                return (0, _utils.PromiseAllSettled)([].concat(_toConsumableArray(callbackFutures), _toConsumableArray(batchCallbackFutures)));

                            case 4:
                                results = _context.sent;
                                rejections = results.filter(_utils.isPromiseAllSettledRejectionResult).map(function (it) {
                                    return it.reason;
                                });
                    ...
                    return regeneratorRuntime.wrap(function _callee2$(_context2) {
                        while (1) {
                            switch (_context2.prev = _context2.next) {
                                case 0:
                                    observableResult = new _ObservableResult.ObservableResultImpl(instrument._descriptor.name, instrument._descriptor.valueType);
                                    callPromise = Promise.resolve(callback(observableResult));

                                    if (timeoutMillis != null) {
                                        callPromise = (0, _utils.callWithTimeout)(callPromise, timeoutMillis);
                                    }
                                    _context2.next = 5;
                                    return callPromise;
                    ...
                    return regeneratorRuntime.wrap(function _callee3$(_context3) {
                        while (1) {
                            switch (_context3.prev = _context3.next) {
                                case 0:
                                    observableResult = new _ObservableResult.BatchObservableResultImpl();
                                    callPromise = Promise.resolve(callback(observableResult));
