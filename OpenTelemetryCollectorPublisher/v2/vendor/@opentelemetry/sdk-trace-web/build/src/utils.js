...
var urlNormalizingAnchor = void 0;
...
function addSpanNetworkEvent(span, performanceName, entries, refPerfName) {
    ...
    var refName = refPerfName || PerformanceTimingNames_1.PerformanceTimingNames.FETCH_START;
...
function addSpanNetworkEvents(span, resource) {
    ...
    var encodedLength = resource[PerformanceTimingNames_1.PerformanceTimingNames.ENCODED_BODY_SIZE];
    if (encodedLength !== undefined) {
        span.setAttribute(semantic_conventions_1.SEMATTRS_HTTP_RESPONSE_CONTENT_LENGTH, encodedLength);
    }
    var decodedLength = resource[PerformanceTimingNames_1.PerformanceTimingNames.DECODED_BODY_SIZE];
...
function sortResources(filteredResources) {
    return filteredResources.slice().sort(function (a, b) {
        var valueA = a[PerformanceTimingNames_1.PerformanceTimingNames.FETCH_START];
        var valueB = b[PerformanceTimingNames_1.PerformanceTimingNames.FETCH_START];
        if (valueA > valueB) {
            return 1;
...
function getResource(spanUrl, startTimeHR, endTimeHR, resources) {
    var ignoredResources = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : new WeakSet();
    var initiatorType = arguments[5];
    ...
    if (parsedSpanUrl.origin !== getOrigin() && sorted.length > 1) {
        ...
        var mainRequest = findMainRequest(sorted, corsPreFlightRequest[PerformanceTimingNames_1.PerformanceTimingNames.RESPONSE_END], endTimeHR);
        var responseEnd = corsPreFlightRequest[PerformanceTimingNames_1.PerformanceTimingNames.RESPONSE_END];
        var fetchStart = mainRequest[PerformanceTimingNames_1.PerformanceTimingNames.FETCH_START];
...
function findMainRequest(resources, corsPreFlightRequestEndTime, spanEndTimeHR) {
    var spanEndTime = (0, core_1.hrTimeToNanoseconds)(spanEndTimeHR);
    var minTime = (0, core_1.hrTimeToNanoseconds)((0, core_1.timeInputToHrTime)(corsPreFlightRequestEndTime));
    var mainRequest = resources[1];
    var bestGap = void 0;
    var length = resources.length;
    for (var i = 1; i < length; i++) {
        var resource = resources[i];
        var resourceStartTime = (0, core_1.hrTimeToNanoseconds)((0, core_1.timeInputToHrTime)(resource[PerformanceTimingNames_1.PerformanceTimingNames.FETCH_START]));
        var resourceEndTime = (0, core_1.hrTimeToNanoseconds)((0, core_1.timeInputToHrTime)(resource[PerformanceTimingNames_1.PerformanceTimingNames.RESPONSE_END]));
...
function filterResourcesForSpan(spanUrl, startTimeHR, endTimeHR, resources, ignoredResources, initiatorType) {
    var startTime = (0, core_1.hrTimeToNanoseconds)(startTimeHR);
    var endTime = (0, core_1.hrTimeToNanoseconds)(endTimeHR);
...
function shouldPropagateTraceHeaders(spanUrl, propagateTraceHeaderCorsUrls) {
    ...
    if (parsedSpanUrl.origin === getOrigin()) {
        ...
        return propagateTraceHeaderUrls.some(function (propagateTraceHeaderUrl) {
            return (0, core_1.urlMatches)(spanUrl, propagateTraceHeaderUrl);
        });
    }
}
