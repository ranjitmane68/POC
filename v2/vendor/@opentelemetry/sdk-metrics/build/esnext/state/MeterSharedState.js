 ...
 */
var MeterSharedState = exports.MeterSharedState = function () {
    function MeterSharedState(_meterProviderSharedState, _instrumentationScope) {
        ...
        this.metricStorageRegistry = new _MetricStorageRegistry.MetricStorageRegistry();
        this.observableRegistry = new _ObservableRegistry.ObservableRegistry();
        this.meter = new _Meter.Meter(this);
    }

    _createClass(MeterSharedState, [{
        key: 'registerMetricStorage',
        value: function registerMetricStorage(descriptor) {
            var storages = this._registerMetricStorage(descriptor, _SyncMetricStorage.SyncMetricStorage);
            if (storages.length === 1) {
                return storages[0];
            }
            return new _MultiWritableMetricStorage.MultiMetricStorage(storages);
        }
    }, {
        key: 'registerAsyncMetricStorage',
        value: function registerAsyncMetricStorage(descriptor) {
            var storages = this._registerMetricStorage(descriptor, _AsyncMetricStorage.AsyncMetricStorage);
            return storages;
        }
        /**
        ...
        value: function _registerMetricStorage(descriptor, MetricStorageType) {
            ...
            if (storages.length === 0) {
                ...
                var collectorStorages = perCollectorAggregations.map(function (_ref2) {
                    ...
                    var storage = new MetricStorageType(descriptor, aggregator, _AttributesProcessor.AttributesProcessor.Noop(), [collector]);
